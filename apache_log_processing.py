...
not200 = access_logs.filter(lambda alog:alog.response_code!=200)
#not200.take(1)

endpointSum = (not200
                          .map(lambda alog:(alog.endpoint,1))
                          .reduceByKey(lambda a,b:a+b))

#endpointSum = endpointCountPairTuple.<FILL IN>

topTenErrURLs = endpointSum.takeOrdered(10,lambda kv:-1*kv[1])
...
uniqueHostCount=(access_logs
 .map(lambda alog:(alog.host,1))
 .reduceByKey(lambda a,b:a+b)
 .map(lambda kv:1)
 .reduce(add))
...
#dt=datetime.strptime("23-aug-2009","%d-%b-%Y")
#print(dt.day)
dailyHosts=(access_logs
            .map(lambda alog:((alog.date_time.day,alog.host),1))
            .reduceByKey(lambda a,b:a+b)
...
daysWithHosts = dailyHosts.map(lambda kv:kv[0]).collect()
hosts = dailyHosts.map(lambda kv:kv[1]).collect()
#print daysWithHosts
#print hosts
...
dayAndHostTuple = (access_logs.map(lambda alog:((alog.date_time.day,alog.host),1))
                   .reduceByKey(lambda a,b:a+b)
                   .map(lambda kv:(kv[0][0],kv[1]))
                   .reduceByKey(lambda a,b:a+b)
                   .sortByKey())
print dayAndHostTuple.take(5)
nhostPerDay=dailyHosts.collect()
dayTotalRequestUniqueHost=dayAndHostTuple.join(dailyHosts)
print dayTotalRequestUniqueHost.take(5)
avgDailyReqPerHost=(dayTotalRequestUniqueHost
...
#avgDailyReqPerHost =dayAndHostTuple
...
daysWithAvg = avgDailyReqPerHost.map(lambda kv:kv[0]).collect()
avgs = avgDailyReqPerHost.map(lambda kv:kv[1]).collect()
...
badRecords = (access_logs
              .filter(lambda alog:alog.response_code==404)
             .cache())
...
badEndpoints = (badRecords.map(lambda alog:(alog.endpoint,1))
                .reduceByKey(lambda a,b:a+b)
               .cache())

badUniqueEndpoints = badEndpoints.map(lambda kv:kv[0])
...
# TODO: Replace <FILL IN> with appropriate code
print badEndpoints.take(5)
badEndpointsCountPairTuple = (badEndpoints.map(lambda kv:(kv[1],kv[0]))
                              .sortByKey(lambda kv:-1*kv[0])
                             .map(lambda kv:kv[1]))

badEndpointsTop20 = badEndpoints.takeOrdered(20,lambda kv:-1*kv[1])
...
errHostsCountPairTuple = (badRecords.map(lambda alog:(alog.host,1))
                        .reduceByKey(lambda a,b:a+b))



errHostsTop25 = errHostsCountPairTuple.takeOrdered(25,lambda kv:-1*kv[1])
...
errDateCountPairTuple = (badRecords.map(lambda alog:(alog.date_time.day,1))
                         .reduceByKey(lambda a,b:a+b))

errDateSorted = errDateCountPairTuple.sortByKey().cache()
...
daysWithErrors404 = errDateSorted.map(lambda kv:kv[0]).collect()
errors404ByDay = errDateSorted.map(lambda kv:kv[1]).collect()
...
topErrDate = errDateSorted.takeOrdered(5,lambda kv:kv[1]*-1)
...
hourRecordsSorted = (badRecords.map(lambda alog:(alog.date_time.hour,1))
                      .reduceByKey(lambda a,b:a+b)
                     .sortByKey()
                     .cache())
...
hoursWithErrors404 = hourRecordsSorted.map(lambda kv:kv[0]).collect()
errors404ByHours = hourRecordsSorted.map(lambda kv:kv[1]).collect()

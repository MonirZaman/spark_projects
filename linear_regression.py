...
def parsePoint(line):
    ...
    """
    words=line.split(",")
    return LabeledPoint(words[0],words[1:])

parsedSamplePoints = rawData.map(lambda x:parsePoint(x))
oneSample=parsedSamplePoints.take(1)[0]
firstPointFeatures = oneSample.features
print firstPointFeatures
print dir(firstPointFeatures[0])

firstPointLabel = oneSample.label
...
# TODO: Replace <FILL IN> with appropriate code
averageTrainYear = float((parsedTrainData
                    .map(lambda x:x.label)
                    .reduce(lambda x,y:x+y)))/parsedTrainData.count()
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return math.sqrt(float(labelsAndPreds.map(lambda x:squaredError(x[0],x[1]))
                   .reduce(lambda x,y:x+y))/labelsAndPreds.count())

labelsAndPreds = sc.parallelize([(3., 1.), (1., 2.), (2., 2.)])
print (labelsAndPreds.map(lambda x:squaredError(x[0],x[1])).reduce(lambda x,y:x+y))
...
labelsAndPredsVal = parsedValData.map(lambda x:(averageTrainYear,x.label))
rmseValBase = calcRMSE(labelsAndPredsVal)

labelsAndPredsTest = parsedTestData.map(lambda x:(averageTrainYear,x.label))
...
print weights.dot(np.array([1.0,.5]))
...
# TODO: Replace <FILL IN> with appropriate code
firstModel = LinearRegressionWithSGD.train(parsedTrainData,iterations=numIters,miniBatchFraction=miniBatchFrac,step=alpha,regType=regType,regParam=reg,intercept=True)
...
#firstModel = LinearRegressionWithSGD.train(parsedTrainData,numIters,alpha,miniBatchFrac,reg,regType,useIntercept)
# weightsLR1 stores the model weights; interceptLR1 stores the model intercept
print firstModel.__getattribute__
...
def twoWayInteractions(lp):
    ...
    """
    prod=[atuple[0]*atuple[1] for atuple in itertools.product(lp.features,lp.features)]
    
    temp=list(lp.features)
    temp.extend(prod)
    print temp
    nfeatures=np.array(temp[:])
    print nfeatures
    
    return LabeledPoint(lp.label,nfeatures)

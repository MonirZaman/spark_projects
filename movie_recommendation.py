...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    return (IDandRatingsTuple[0],(len(IDandRatingsTuple[1]),float(sum(IDandRatingsTuple[1]))/len(IDandRatingsTuple[1])))
...
movieIDsWithAvgRatingsRDD = movieIDsWithRatingsRDD.map(lambda x:getCountsAndAverages((x[0],tuple(x[1]))))
...
def computeError(predictedRDD, actualRDD):
    ...
    predictedReformattedRDD = predictedRDD.map(lambda (uid,mid,rating):((uid,mid),rating))

    # Transform actualRDD into the tuples of the form ((UserID, MovieID), Rating)
    actualReformattedRDD = actualRDD.map(lambda (uid,mid,rating):((uid,mid),rating))

    # Compute the squared error for each matching entry (i.e., the same (User ID, Movie ID) in each
    # RDD) in the reformatted RDDs using RDD transformtions - do not use collect()
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD)
                       .map(lambda x:pow(abs(x[1][0]-x[1][1]),2)))
...
trainingAvgRating = float(trainingRDD.map(lambda x:x[2]).reduce(lambda a,b:a+b))/trainingRDD.count()
...
movieIListed=myRatingsRDD.map(lambda x:x[1]).collect()
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda x:x[0] not in movieIListed)
                     .map(lambda x:(0,x[0])))

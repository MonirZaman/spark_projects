...
print simpleTokenize('!!!!123A/456_B/789C.123A') # Should give ['a', 'quick', 'brown', ... ]
...
def tokenize(string):
    ...
    if string ==" " or string=="":
        return []
    words=filter(lambda x:(x!='')&(x not in stopwords),re.split(split_regex,string.lower()))

    #splittedWords=
    #stoprdd=sc.textFile(stopfile)
    
    #fword=(stoprdd.flatMap(lambda x:[(aw,1) if aw !=x else (aw,0) for aw in words])
    #       .reduceByKey(lambda a,b:a*b)
    #       .filter(lambda x:x[1]!=0)
    #       .map(lambda x:x[0]))
    #adict={'quick':0,'brown':1,'fox':2,'jumps':3,'lazy':4,'dog':5}
    #return sorted(fword.collect(),key=lambda x:adict[x])
    return words
...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.flatMap(lambda x:[(x[0],aword) for aword in tokenize(x[1])])
#print amazonRecToToken.count()
googleRecToToken = googleSmall.flatMap(lambda x:[(x[0],aword) for aword in tokenize(x[1])])
...
def findBiggestRecord(vendorRDD):
    ...
    return (vendorRDD.map(lambda x:(x[0],1))
            .groupByKey()
            .takeOrdered(1,lambda x:len(x[1])*-1))
...
def tf(tokens):
    ...
    for tk in adict.keys():
        adict[tk]=float(adict[tk])/len(tokens)
    return adict
...
a=amazonRecToToken.map(lambda x:(x[1],x[0]))
g=googleRecToToken.map(lambda x:(x[1],x[0]))
#print a.join(g).take(5)
print amazonRecToToken.take(15)
...
def idfs(corpus):
    ...
    """
    N = amazonSmall.count()+googleSmall.count()
    uniqueTokens = (corpus.map(lambda x:(x[1],1))
                    .reduceByKey(lambda a,b:a+b)
                    .map(lambda x:x[0]))
    #find how many doc contains a token
    tokenFreq=(corpus.map(lambda x:(x,1))
     .reduceByKey(lambda a,b:a+b)
     .map(lambda x:(x[0][1],1))
     .reduceByKey(lambda a,b:a+b)
     .map(lambda x:(x[0],float(N)/x[1])))
...
def tfidf(tokens, idfs):
    ...
    #print adict
    #print tokens
    for atoken in adict.keys():
        #print idfs[atoken]
        adict[atoken]=(float(adict[atoken])/len(tokens))*idfs[atoken]
        
    return adict

#recb000hkgj8k = amazonRecToToken.filter(lambda x: x[0] == 'b000hkgj8k').collect()[0][1]
recb000hkgj8k = amazonRecToToken.filter(lambda x: x[0] == 'b000hkgj8k').map(lambda x:(x[1])).collect()
...
def dotprod(a, b):
    ...
    for akey in allkeys:
        ...
        if b.has_key(akey):
            bv=b[akey]
        sv+=(av*bv)
    #return sum([a[k]*b[k] for i,k in enumerate(a.keys())])
    return sv
def norm(a):
...
#print len(idfsSmallWeights.keys())
amazonTokens=amazon.flatMap(lambda x:[(x[0],aword) for aword in tokenize(x[1])])
googleTokens=google.flatMap(lambda x:[(x[0],aword) for aword in tokenize(x[1])])
bigCorpus=amazonTokens.union(googleTokens)
idfsBigWeights=idfs(bigCorpus).collectAsMap()

print len(idfsBigWeights.keys())
print idfsBigWeights.keys()[:5]
print idfsBigWeights['adobe']
print idfsBigWeights['illustrator']
print idfsBigWeights['photoshop']
...
def similar(amazonID, googleURL):
    ...
    filteredRdd= (crossSmall
            .filter(lambda record: (record[0][0] == googleURL and record[1][0] == amazonID)))
    
    #print filteredRdd.count()
    lst=filteredRdd.collect()
    #print lst
    if len(lst)==1:
        return computeSimilarity(lst[0])[2]
    else:
...
def similarBroadcast(amazonID, googleURL):
    ...
    #print lst
    if len(lst)==1:
        return computeSimilarityBroadcast(lst[0])[2]
    else:
        print "List contains more than one element"
...
# TODO: Replace <FILL IN> with appropriate code
sims = (crossSmall.map(lambda rec:(rec[1][0]+" "+rec[0][0],cosineSimilarity(rec[0][1],rec[1][1], idfsSmallBroadcast.value))))
#print sims.take(13)

trueDupsRDD = (sims
               .join(goldStandard))
trueDupsCount = trueDupsRDD.count()
avgSimDups = (trueDupsRDD.map(lambda x:x[1][0])
             .reduce(lambda a,b:a+b))

avgSimDups=float(avgSimDups)/trueDupsCount
...
avgSimNon = float(nonDupsRDD.reduce(lambda a,b:a+b))/nonDupsCount
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.flatMap(lambda x:[(x[0],aword) for aword in tokenize(x[1])])

googleFullRecToToken = google.flatMap(lambda x:[(x[0],aword) for aword in tokenize(x[1])])
print 'Amazon full dataset is %s products, Google full dataset is %s products' % (amazonFullRecToToken.count(),
                                                                                    googleFullRecToToken.count())
print amazonRecToToken.count()
print googleRecToToken.count()
...
#amazonWeightsRDD = tfidf(amazonFullRecToToken.map(lambda x:x[1]).collect(),idfsFullBroadcast.value)
...
#print amazonNorms.collectAsMap().
...
def invert(record):
    ...
    return [(akey,record[0]) for akey in record[1].keys()]
...
broadcastVar=[amazonWeightsBroadcast.value, googleWeightsBroadcast.value]
bvNorms=[amazonNormsBroadcast.value,googleNormsBroadcast.value]
def fastCosineSimilarity(record,broadcastVar,bvNorms):
    ...
    keys=[record[0][0],record[0][1]]
    """
    dictVar=[{},{}]
    
    for atoken in record[1]:
        dictVar[0][atoken]=broadcastVar[0][keys[0]][atoken]
        dictVar[1][atoken]=broadcastVar[1][keys[1]][atoken]
    """
    sv=0
    
    for atoken in record[1]:
        sv+=(broadcastVar[0][keys[0]][atoken]*broadcastVar[1][keys[1]][atoken])
    
    denom=amazonNormsBroadcast.value[keys[0]]*googleNormsBroadcast.value[keys[1]]
    return (record[0], sv/denom)

similaritiesFullRDD = (commonTokens
                       .map(lambda x:fastCosineSimilarity(x,broadcastVar,bvNorms))
                       .cache())
print similaritiesFullRDD.take(10)
